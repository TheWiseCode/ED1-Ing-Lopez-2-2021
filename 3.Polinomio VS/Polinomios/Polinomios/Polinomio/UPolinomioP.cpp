...
NodoPo* PolinomioP::buscar_exponente(int exp) {
	NodoPo * aux = ptr_poli;
	while (aux != NULL) {
		if (aux->exp == exp)
			return aux;
...
NodoPo* PolinomioP::buscar_termino_n(int i) {
	int c = 1;
	NodoPo * aux = ptr_poli;
	while (aux) {
		if (c == i)
...
NodoPo* PolinomioP::anterior(NodoPo* dir) {
	NodoPo* aux = ptr_poli;
	NodoPo* ant = NULL;
	while (aux && aux != dir) {
		ant = aux;
...
void PolinomioP::suprime(NodoPo* dir) {
	if (dir == ptr_poli) {
...
void PolinomioP::asignar_coeficiente(int coef, int exp) {
	NodoPo* dir = buscar_exponente(exp);
	if (dir) {
		dir->coef = coef;
		if (coef == 0) {
...
int PolinomioP::exponente(int nro_ter) {
...
void PolinomioP::mostrar_integral() {
	for (int i = 0; i < numero_terminos(); i++) {
		int exp = exponente(i + 1);
		int coef = coeficiente(exp);
...
PolinomioP* PolinomioP::suma(PolinomioP* a, PolinomioP* b) {
	PolinomioP* p = new PolinomioP();
...
PolinomioP* PolinomioP::resta(PolinomioP* a, PolinomioP* b) {
	PolinomioP* p = new PolinomioP();
	for (int i = 0; i < a->numero_terminos(); i++) {
		int exp = a->exponente(i + 1);
		int coef = a->coeficiente(exp);
...
PolinomioP* PolinomioP::producto(PolinomioP* a, PolinomioP* b) {
	PolinomioP* p = new PolinomioP();
