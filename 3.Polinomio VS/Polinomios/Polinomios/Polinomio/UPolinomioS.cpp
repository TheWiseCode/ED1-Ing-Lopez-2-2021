...
PolinomioS::PolinomioS(CSMemoria* m) {
	mem = m;
	ptr_poli = NULO;
	nt = 0;
}

int PolinomioS::buscar_exponente(int exp) {
	int aux = ptr_poli;
	while (aux != NULO) {
...
int PolinomioS::buscar_termino_n(int i) {
	int c = 1;
	int aux = ptr_poli;
	while (aux != NULO) {
		if (c == i)
...
int PolinomioS::anterior(int dir) {
	int aux = ptr_poli;
	int ant = NULO;
...
void PolinomioS::suprime(int dir) {
	if (dir == ptr_poli) {
...
void PolinomioS::asignar_coeficiente(int coef, int exp) {
	int dir = buscar_exponente(exp);
	if (dir != NULO) {
...
int PolinomioS::exponente(int nro_ter) {
...
void PolinomioS::mostrar_integral() {
