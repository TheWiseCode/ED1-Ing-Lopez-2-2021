...
int CSMemoriaZZZ::new_espacio(string cad) {
...
void CSMemoriaZZZ::delete_espacio(int dir) {
	int x = dir;
	while(mem[x].link != -1)
		x = mem[x].link;
	mem[x].link = libre;
...
void CSMemoriaZZZ::poner_dato(int dir, string id, int valor) {
	int z = dir;
...
int CSMemoriaZZZ::obtener_dato(int dir, string lugar) {
	int z = dir;
...
int CSMemoriaZZZ::espacio_disponible() {
	int x = libre;
	int c = 0;
	while(x != -1){
		c++;
...
int CSMemoriaZZZ::espacio_ocupado() {
	return MAX - espacio_disponible();
}

bool CSMemoriaZZZ::dir_libre(int dir) {
	int x = libre;
	bool c = false;
	while(x != -1 && c == false){
		if(x == dir)
